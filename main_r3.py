...
class PostavkeGeneriranja:
    ...
    def dohvatiBrojRacunala(self):
        ...
        self.cvorovi = list(
            map(chr, range(97, 97 + self.brojRacunala)))  # riskirat ćemo i reći da nema više od 26 čvorova
        print(self.cvorovi)
...
def izracunajNajkraciPut(G):
    cvoroviNaNajkracem = nx.shortest_path(G, source=postavke.pocetakSirenja, target=postavke.krajSirenja,
                                          weight='weight')
...
def izracunajHamiltonoveCikluse(G):
    ...
    unikatniCiklusi = set(tuple(i) for i in ciklusi) #just in case
